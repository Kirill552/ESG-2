 ...
 * Источник: Распоряжение Правительства РФ от 22.10.2021 № 2979-р
 * (в редакции Распоряжения от 04.04.2025 № 805-р)
 * Основано на IPCC AR6 (Sixth Assessment Report)
 */
export const GLOBAL_WARMING_POTENTIAL = {
  ...
  SF6: 23500,  // Гексафторид серы (SF₆)
};

/**
 * Потенциалы глобального потепления для конкретных ГФУ (HFC)
 * Источник: Распоряжение 2979-р (в ред. 805-р), IPCC AR6
 */
export const HFC_SUBSTANCES = {
  // Трифторметан (HFC-23)
  'CHF3': { gwp: 14800, name: 'Трифторметан (HFC-23)', formula: 'CHF₃' },
  'HFC-23': { gwp: 14800, name: 'Трифторметан (HFC-23)', formula: 'CHF₃' },

  // Дифторметан (HFC-32)
  'CH2F2': { gwp: 675, name: 'Дифторметан (HFC-32)', formula: 'CH₂F₂' },
  'HFC-32': { gwp: 675, name: 'Дифторметан (HFC-32)', formula: 'CH₂F₂' },

  // Пентафторэтан (HFC-125)
  'C2HF5': { gwp: 3500, name: 'Пентафторэтан (HFC-125)', formula: 'C₂HF₅' },
  'HFC-125': { gwp: 3500, name: 'Пентафторэтан (HFC-125)', formula: 'C₂HF₅' },

  // 1,1,1,2-Тетрафторэтан (HFC-134a) - самый распространенный хладагент
  'CH2FCF3': { gwp: 1430, name: '1,1,1,2-Тетрафторэтан (HFC-134a)', formula: 'CH₂FCF₃' },
  'HFC-134a': { gwp: 1430, name: '1,1,1,2-Тетрафторэтан (HFC-134a)', formula: 'CH₂FCF₃' },

  // 1,1,1-Трифторэтан (HFC-143a)
  'C2H3F3': { gwp: 4470, name: '1,1,1-Трифторэтан (HFC-143a)', formula: 'C₂H₃F₃' },
  'HFC-143a': { gwp: 4470, name: '1,1,1-Трифторэтан (HFC-143a)', formula: 'C₂H₃F₃' },

  // 1,1-Дифторэтан (HFC-152a)
  'C2H4F2': { gwp: 124, name: '1,1-Дифторэтан (HFC-152a)', formula: 'C₂H₄F₂' },
  'HFC-152a': { gwp: 124, name: '1,1-Дифторэтан (HFC-152a)', formula: 'C₂H₄F₂' },
};

/**
 * Потенциалы глобального потепления для конкретных ПФУ (PFC)
 * Источник: Распоряжение 2979-р (в ред. 805-р), IPCC AR6
 */
export const PFC_SUBSTANCES = {
  // Тетрафторметан (PFC-14)
  'CF4': { gwp: 7380, name: 'Тетрафторметан (PFC-14)', formula: 'CF₄' },
  'PFC-14': { gwp: 7380, name: 'Тетрафторметан (PFC-14)', formula: 'CF₄' },

  // Гексафторэтан (PFC-116)
  'C2F6': { gwp: 12200, name: 'Гексафторэтан (PFC-116)', formula: 'C₂F₆' },
  'PFC-116': { gwp: 12200, name: 'Гексафторэтан (PFC-116)', formula: 'C₂F₆' },

  // Октафторпропан (PFC-218)
  'C3F8': { gwp: 8830, name: 'Октафторпропан (PFC-218)', formula: 'C₃F₈' },
  'PFC-218': { gwp: 8830, name: 'Октафторпропан (PFC-218)', formula: 'C₃F₈' },

  // Декафторбутан (PFC-31-10)
  'C4F10': { gwp: 8860, name: 'Декафторбутан (PFC-31-10)', formula: 'C₄F₁₀' },
  'PFC-31-10': { gwp: 8860, name: 'Декафторбутан (PFC-31-10)', formula: 'C₄F₁₀' },

  // Додекафторпентан (PFC-41-12)
  'C5F12': { gwp: 9160, name: 'Додекафторпентан (PFC-41-12)', formula: 'C₅F₁₂' },
  'PFC-41-12': { gwp: 9160, name: 'Додекафторпентан (PFC-41-12)', formula: 'C₅F₁₂' },

  // Тетрадекафторгексан (PFC-51-14)
  'C6F14': { gwp: 7910, name: 'Тетрадекафторгексан (PFC-51-14)', formula: 'C₆F₁₄' },
  'PFC-51-14': { gwp: 7910, name: 'Тетрадекафторгексан (PFC-51-14)', formula: 'C₆F₁₄' },
};
...
/**
 * Коэффициенты выбросов для транспорта (в кг CO₂/л)
 * Задача 10.6.1 из OCR-REPORTS.md
 *
 * Расчет: Приказ №371 (т CO₂/т топлива) × плотность (кг/л) / 1000
 */
...
/**
 * Типовые нормы расхода топлива по типам ТС (л/100 км)
 * Источник: Приказ Минтранса РФ от 14.03.2008 N АМ-23-р
 */
export const VEHICLE_CONSUMPTION_ESTIMATES: Record<string, number> = {
  'газель': 12.5,
  'камаз': 28.0,
  'фура': 35.0,
  'легковой': 8.0,
  ...
  'грузовик': 25.0,
  'тягач': 32.0,
  // Дефолтное значение для неизвестных моделей
  'default': 20.0
};
